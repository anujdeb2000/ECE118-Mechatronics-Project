...
typedef enum {
    ...
    EdgeWait,
    EndState,
    TheyHitTheSecondObstacleR,
    TheyHitTheSecondObstacleL,
    SecondPartTwo,
    WeNeedToMove,
...
ES_Event RunObstacleSubHSM(ES_Event ThisEvent)
{
    ...
    case BackWait: // in the first state, replace this with correct names
    ...
    case TopCheck: 
        switch (ThisEvent.EventType) {
            ...
            case RIGHT_OBSTACLE_ON: 
                     ...
                     BackwardMotorSpeed(GENERAL_MOTOR_SPEED);
                     nextState = TheyHitTheSecondObstacleR;
                     makeTransition = TRUE;
                     ThisEvent.EventType = ES_NO_EVENT;
            ...
            case LEFT_OBSTACLE_ON: 
                     ...
                     ES_Timer_InitTimer(LMAO_TIMER, 500);
                     BackwardMotorSpeed(GENERAL_MOTOR_SPEED);
                     nextState = TheyHitTheSecondObstacleL;
                     makeTransition = TRUE;
                     ThisEvent.EventType = ES_NO_EVENT;
        ...
        case TheyHitTheSecondObstacleR: 
        switch (ThisEvent.EventType) {
            case ES_TIMEOUT: 
                if(ThisEvent.EventParam == LMAO_TIMER){
        ...
        case TheyHitTheSecondObstacleL: 
        switch (ThisEvent.EventType) {
            case ES_TIMEOUT: 
                if(ThisEvent.EventParam == LMAO_TIMER){
                     ES_Timer_InitTimer(CORNER_TIMER, 250);
        ...
        switch (ThisEvent.EventType) {
            case ES_TIMEOUT: 
                if(ThisEvent.EventParam == LMAO_TIMER){
                     ForwardMotorSpeed(GENERAL_MOTOR_SPEED);
                     ES_Timer_InitTimer(TAPE_WAIT_TIMER, (TAPE_WAIT_TIME * 0.25) );
                     nextState = TopCheck;
                     makeTransition = TRUE;
                     ThisEvent.EventType = ES_NO_EVENT;
                }
