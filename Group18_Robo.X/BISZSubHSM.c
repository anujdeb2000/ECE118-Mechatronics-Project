...
static BISZSubHSMState_t CurrentState = InitPSubState; // <- change name to match ENUM
...
uint8_t InitTemplateSubHSM(void)
{
    ...
    returnEvent = RunBISZSubHSM(INIT_EVENT);
...
ES_Event RunBISZSubHSM(ES_Event ThisEvent)
{
    uint8_t makeTransition = FALSE; // use to flag transition
    BISZSubHSMState_t nextState; // <- change type to correct enum
...
//                ThisEvent.EventType = ES_NO_EVENT;
    ...
    if (makeTransition == TRUE) { // making a state transition, send EXIT and ENTRY
        ...
        RunBISZSubHSM(EXIT_EVENT); // <- rename to your own Run function
        CurrentState = nextState;
        RunBISZSubHSM(ENTRY_EVENT); // <- rename to your own Run function
    }
